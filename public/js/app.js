...
$(function() {
    $("#btn-save").click(function (e) {
        ...
        var refernceNo = jQuery('#refernceNo').val();
        ...
        if(cnic == '' || refernceNo == '') {
          Swal.fire({
            ...
            icon: 'error',
            title: 'Error',
            text: "Please Enter CNIC or Reference No First", 
        ...
        var formData = {
            cnic: jQuery('#cnic').val(),
            refernceNo: jQuery('#refernceNo').val(),
            dealer_id: jQuery('#dealer_id').val(),
            agency_name: jQuery('#agency_name').val(),
            owner: jQuery('#owner').val(),
        };
           
        var type = "POST";
        var ajaxurl = 'showPlotStatus';
        $.ajax({
            type: type,
            beforeSend: function(){
            ...
            success: function (data) {
                ...
                if(data.status == 'error') {
                    ...
                    var html = '<div class="card card-block " style="max-height: 350px!important;">';
                    html += '<div style="margin-bottom: 25px;">';
                    html += '<h4>Plot Details</h4>'; 
                    html += '</div>';
                    html += '<span class="errorMsg" style="color:red;">'+data.data+'</span>'; 
                    html += '</div>';

                    var html2 = '<fieldset class="col-sm-12 col-md-12">';
                    // html2 += '<span>You Can Search <b>6 Plots Verification </b> in a day.</span><br/>';
                    html2 += '<span>Search Limit: <b>'+data.dailyUsed+' / 30 </span><br/>';
                    // html2 += '<span>You Have Used <span style="color:red">'+data.dailyUsed+'</span><b> Plots Verification </b> For Today.</span>';
                    html2 += '</fieldset>';
                    
                    $("#dailyUsed").html(html2);
                    $(".ledgerDetail").html(html);
                    // $(".ledgerDetail").hide();
                    // $(".errorMsg").text(data.data);
                    // $(".errorMsg").show();
                  }
                  if(data.status == 'success') {
                    // var html = '<div class="col-md-6">';
                    var html = '<div class="card card-block " style="max-height: 420px!important;">';
                    html += '<div style="margin-bottom: 15px;">';
                    html += '<h4>Plot Details</h4>'; 
                    html += '</div>';
                    html += '<article class="card">'; 
                    html += '<span class="errorMsg" style="color:red;"></span>'; 
                    html += '<ul class="list-group">';
                    html += '<li class="list-group-item-custom"><label class="">Plot No:</label>';
                    html += '<span style="float: right;">';
                    html += data.data[0]['Plot_No'];
                    html += '</span></li>';
                    html += '<li class="list-group-item-custom"><label class="">Plot Status:</label>';
                    
                    if(data.data[0]['PlotStatus'] == 'Active') 
                    {
                      html += '<span style="float: right; color: #0cc20c">';
                      html += data.data[0]['PlotStatus'];
                      html += '</span></li>';
                    }
                    else 
                    {
                      html += '<span style="float: right; color: #dc3912">';
                      html += data.data[0]['PlotStatus'];
                      html += '</span></li>';
                    }
                    $.each(data.data, function(k, v) {
                      html += '<li class="list-group-item-custom"><label class="">'+ v['PaymentTypeTitle']+':</label>';
                      html += '<span style="float: right;"> <b>Rs: </b>'+ addCommas(v['PaidAmount'])+' / '+ addCommas(v['Amount']) +' </span></li>';
                    });
                    html += '</ul>';
                    // html += '<form action="http://localhost/new_Dealer/getChallanField" method="get" >';
                    // html += '<input type="hidden" value="'+data.data[0]['MemberID']+'" name="appId" id="appId" >';
                    // html += '<button class="btn btn-primary" style="margin-top:25px;" >Generate Challan</button>';
                    // html += '</form>';
                    html += '</div>';
                    html += '<div>';
                    ...
                    var html2 = '<fieldset class="col-sm-12 col-md-12">';
                    html2 += '<span style="color:red;">Per Day Search Limit: <b>'+data.dailyUsed+' / 30 </span><br/>';
                    //html2 += '<span>You Have Used <b><span style="color:red">'+data.dailyUsed+' Plots Verification </span></b> For Today.</span>';
                    html2 += '</fieldset>';
                    // html += '</div>';
                
                    // $(".errorMsg").hide();
                    // $(".ledgerDetail").show();
                    //var dailyUsedTxt = 'You Have Used <b>'+data.dailyUsed+' Plots Verification </b> For Today.';
                    $("#dailyUsed").html(html2);
                    $(".ledgerDetail").html(html);
                  }
                },
    ...
    $("#btn-save-challan").click(function (e) {
        var cnic =  jQuery('#cnic').val();
        var refernceNo = jQuery('#refernceNo').val();
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if(data.status == 'success') {
                    // var html = '<div class="col-md-6">';
                    var html = '<div class="card card-block " style="max-height: 380px!important;">';
                    html += '<div style="margin-bottom: 25px;">';
                    html += '<h4>Plot Details</h4>'; 
                    html += '</div>';
                    html += '<article class="card">'; 
